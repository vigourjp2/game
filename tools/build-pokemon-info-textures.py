...
FIELD3D_GENERATOR_VERSION = 'REAL_96X96X96_SOURCE_VOLUME_V8_VERSIONED_B64_NO_FAKE_PARTS'
FIELD3D_GRID = 96
...
def compact_field3d_cells(cells: List[Dict]) -> Dict[str, Any]:
    """field3dをGitHub 100MB制限対策で圧縮保存する。

    形状は96x96x96のまま落とさない。保存形式だけ、
    verbose JSON object配列から palette + base64 binary に変換する。
    1セル=5byte: x,y,z,colorIndex16。
    """
    palette: List[str] = []
    index: Dict[str, int] = {}
    raw = bytearray()
    for c in cells:
        try:
            x = max(0, min(255, int(c.get('x', 0))))
            y = max(0, min(255, int(c.get('y', 0))))
            z = max(0, min(255, int(c.get('z', 0))))
        except Exception:
            continue
        color = str(c.get('color', '#888888')).lower()
        if not re.fullmatch(r'#[0-9a-fA-F]{6}', color):
        ...
        if ci is None:
            ...
            if ci >= 65535:
                ...
                color = _rgb_to_hex(tuple(int(round(v / 8) * 8) for v in rgb))
                ci = index.get(color)
                if ci is None:
                    ci = len(palette)
                ...
                palette.append(color)
                index[color] = ci
        raw.extend((x, y, z, (ci >> 8) & 255, ci & 255))
    return {
        'encoding': 'b64_xyzc16_v1',
        'cellCount': len(raw) // 5,
        'palette': palette,
...
def build(input_dir: Path, out_file: Path, grid: int, safe_pad: float = 0.22, scale: float | None = None) -> List[Dict]:
    ...
    for p in files:
        ...
        try:
            ...
            compact3d = compact_field3d_cells(field3d_cells)
            spec = {'archetype': FIELD3D_GENERATOR_VERSION, 'modelScale': 1.0, 'features': ['real_96x96x96_source_volume', 'no_fake_parts', 'no_16x16_downscale', 'generated_from_cells', 'compact_b64_storage']}
            rows.append({
                ...
                'field3d': {
                    ...
                    'encoding': compact3d['encoding'],
                    'cellCount': compact3d['cellCount'],
                    'palette': compact3d['palette'],
                    'data': compact3d['data'],
  ...
  function decodeField3dCells(f){{
    ...
    if(f.encoding==='b64_xyzc16_v1' && typeof f.data==='string'){{
      const pal=Array.isArray(f.palette)?f.palette:[]; const bin=atob(f.data); const out=[];
      for(let i=0;i+4<bin.length;i+=5){{ const ci=(bin.charCodeAt(i+3)<<8)|bin.charCodeAt(i+4); out.push({{x:bin.charCodeAt(i),y:bin.charCodeAt(i+1),z:bin.charCodeAt(i+2),color:normalizeHex(pal[ci]),size:scale}}); }}
      f.cells=out; return out;
    }}
    return [];
  }}
  function buildField3d(d, plane){{
    const f=d&&d.field3d; const decoded=decodeField3dCells(f); if(!f || !decoded.length) return null;
