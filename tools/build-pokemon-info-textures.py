...
- generated/pokemon-field3d-lite.v11.generated.js へ互換出力

使い方:
  python tools/build-pokemon-info-textures.py
  python tools/build-pokemon-info-textures.py --grid 96 --safe-pad 0.22 --input assets/pokemon-gen1-icons --out generated/pokemon-field3d-lite.v11.generated.js
...
FIELD3D_GENERATOR_VERSION = 'POKEMON_FIELD3D_LITE_V11_COMPACT_96_VOLUME_NO_FLAT_BILLBOARD'
FIELD3D_GRID = 96
...
def build(input_dir: Path, out_file: Path, grid: int, safe_pad: float = 0.22, scale: float | None = None) -> List[Dict]:
    ...
    # v11: 本番を軽くするため、平面 cells は出力しない。field3d compact だけ出す。
    lite = []
    for d in rows:
        f = d.get('field3d') or {}
        lite.append({
            'no': d.get('no'),
            'id': f.get('id') or ('info-poke3d_%03d' % int(d.get('no') or 0)),
            'planeId': d.get('id'),
            'type': d.get('type'),
            'name': d.get('name'),
            'grid': f.get('grid') or FIELD3D_GRID,
            'scale': f.get('scale') or round(2.55 / float(FIELD3D_GRID), 6),
            'encoding': f.get('encoding'),
            'cellCount': f.get('cellCount'),
            'palette': f.get('palette') or [],
            'data': f.get('data') or '',
            'archetype': FIELD3D_GENERATOR_VERSION,
            'features': ['field3d_only','no_plane_billboard_payload','lazy_decode_in_index','lod_render','no_fake_parts']
        })
    payload = json.dumps(lite, ensure_ascii=False, separators=(',', ':'))
    js = """// Auto-generated by tools/build-pokemon-info-textures.py. Do not hand edit.\n// Generator: %s; field3d-only v11; no flat billboard payload.\n// Output: generated/pokemon-field3d-lite.v11.generated.js\n(function(){\n  const defs = %s;\n  const byNo = Object.create(null);\n  for (const d of defs) { byNo[String(d.no).padStart(3,'0')] = d; }\n  window.POKEMON_FIELD3D_LITE_V11 = { version: '%s', defs, byNo, source: 'generated/pokemon-field3d-lite.v11.generated.js' };\n})();\n""" % (FIELD3D_GENERATOR_VERSION, payload, FIELD3D_GENERATOR_VERSION)
    out_file.write_text(js, encoding='utf-8')
    print(f'WROTE {out_file} entries={len(lite)} field3d_only_v11 grid={FIELD3D_GRID} no_plane_cells safe_pad={safe_pad}')
    return rows
...
def main() -> None:
    ...
    ap.add_argument('--out', default='generated/pokemon-field3d-lite.v11.generated.js')
